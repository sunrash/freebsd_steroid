...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=/root/JP/openssl-chelsio_toe_1_1_1//.openssl no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-threads no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "amd64-freebsd-thread-multi",
  perl_cmd => "/usr/local/bin/perl",
  perl_version => "5.28.1",
  perlargv => [ "BSD-x86_64", "--prefix=/root/JP/openssl-chelsio_toe_1_1_1//.openssl", "no-shared", "no-threads" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
  ...
  prefix => "/root/JP/openssl-chelsio_toe_1_1_1//.openssl",
  processor => "",
