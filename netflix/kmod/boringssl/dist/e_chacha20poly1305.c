				       ...
				       uint8_t *tagout, size_t *tagsize)
{
  ...
  for (i=0; i<in_iov_cnt; i++) {
	  ...
	  inlen = iovin[i].iov_len;
	  out = (uint8_t *)iovout[i].iov_base;
	  CRYPTO_chacha_20(out, in, inlen, c20_ctx->key, nonce, 1);
	  poly1305_update_with_length(&poly1305, out, inlen);
	  iovout[i].iov_len = iovin[i].iov_len;
  }
  CRYPTO_poly1305_finish(&poly1305, tag);
  memcpy(tagout, tag, c20_ctx->tag_len);
  *tagsize = c20_ctx->tag_len;
  return 1;
}
