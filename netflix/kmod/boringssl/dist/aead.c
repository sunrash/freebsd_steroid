                      ...
                      const uint8_t *ad, size_t ad_len)
{
	...
	struct iovec iniov[2];
	struct iovec outiov[2];
	size_t taglen;

	iniov[0].iov_base = ((uint8_t *)(__uintptr_t)(const void *)(in));
	iniov[0].iov_len = in_len;
	iniov[1].iov_base = NULL;
	iniov[1].iov_len = 0;
	outiov[0].iov_base = out;
	outiov[0].iov_len = max_out_len;
	...
	taglen = max_out_len-in_len;
	if (ctx->aead->seal(ctx, outiov, 1, nonce, nonce_len, iniov,
			    1, ad, ad_len, out+in_len, &taglen)) {
		return 1;
	}
